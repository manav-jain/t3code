...
export const ExecutionEnvironmentCapabilities = Schema.Struct({
  ...
  worktreeCleanupOnSettle: Schema.optionalKey(Schema.Boolean),
  ...
  /** Server understands groupName on thread.meta.update. Absent on older
      servers, so clients hide group actions instead of sending them. */
  ...
  /** Server understands thread.slack-thread.link / .unlink, exposes `slackThreads` on threads,
      and serves slack.thread.read. Same version-skew contract as threadSettlement. */
