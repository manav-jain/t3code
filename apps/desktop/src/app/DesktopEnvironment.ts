...
export function resolveDesktopAppBranding(input: {
  ...
  return {
    ...
    stageLabel,
    // Fork build: a distinct name so it installs beside the official app.
    displayName: `${APP_BASE_NAME} (${stageLabel === "Alpha" ? "MJ" : stageLabel})`,
  };
}
...
const make = Effect.fn("desktop.environment.make")(function* (
  ...
  const userDataDirName = isDevelopment ? "t3code-dev" : "t3code-mj";
  const legacyUserDataDirName = isDevelopment ? "T3 Code (Dev)" : "T3 Code (MJ)";
  ...
  return DesktopEnvironment.of({
    ...
    appUserModelId: Option.getOrElse(config.appUserModelIdOverride, () =>
      isDevelopment ? "com.t3tools.t3code.dev" : "com.t3tools.t3code.mj",
    ),
